...
import styles from '../../assets/styles/jobDetails.styles'; // می‌تونی یه فایل جدید بسازی مثل clouteDetails.styles
...
export default function ClouteDetails() {
  ...
  const saveItem = async () => {
    try {
      ...
      const res = await apiFetch("/saved-ads", {
        ...
        body: JSON.stringify({ adId: parsedItem._id, adType: "cloutes" })
      });
      ...
      console.error("Error saving cloute item:", error);
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.details}>
        <View style={styles.infoBox}>
          ...
          {parsedItem.cloutesModel && (
            <>
              <View style={styles.infoRow}>
                <Ionicons name="shirt-outline" size={20} color={COLORS.primary} style={styles.icon} />
                <Text style={styles.info}>مدل: {parsedItem.cloutesModel}</Text>
              </View>
              <View style={styles.separator} />
            </>
          )}

          {parsedItem.cloutesStatus && (
            <>
              <View style={styles.infoRow}>
                <Ionicons name="checkmark-done-outline" size={20} color={COLORS.primary} style={styles.icon} />
                <Text style={styles.info}>وضعیت: {parsedItem.cloutesStatus}</Text>
              </View>
              <View style={styles.separator} />
            </>
          )}

          {parsedItem.cloutesTexture && (
            <>
              <View style={styles.infoRow}>
                <Ionicons name="layers-outline" size={20} color={COLORS.primary} style={styles.icon} />
                <Text style={styles.info}>جنس: {parsedItem.cloutesTexture}</Text>
              </View>
              <View style={styles.separator} />
            </>
          )}
